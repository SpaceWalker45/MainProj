...
def main(image_path):
    with mp_face_mesh.FaceMesh(max_num_faces=1, refine_landmarks=True, min_detection_confidence=0.5,
        ...
        if results.multi_face_landmarks:
            ...
            cv2.putText(frame, f"LOOK STRAIGHT AT THE CENTRE", (170, 150), cv2.FONT_HERSHEY_PLAIN, 1.2, (0, 255, 0),
                        1, cv2.LINE_AA)
            cv2.putText(frame, f"Deviation : {total_ratio:.2f}", (30, 30), cv2.FONT_HERSHEY_PLAIN, 1.2, (0, 0, 255),
                        1, cv2.LINE_AA)
            cv2.drawMarker(frame, (img_w // 2, img_h // 2), (0, 255, 0), markerType=cv2.MARKER_CROSS, markerSize=30,
